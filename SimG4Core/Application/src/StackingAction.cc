...
G4ClassificationOfNewTrack StackingAction::ClassifyNewTrack(const G4Track *aTrack) {
    ...
    if (aTrack->GetCreatorProcess() == 0
        ...
        // ten if moglby zostac jak jest- bo ke i tak potrzebuje aTrack, a pdg juz mam
        ...
        else if (!trackNeutrino &&
                 (abspdg == 12 || abspdg == 14 || abspdg == 16 || abspdg == 18)) { classification = fKill; }
        else if (classification != fKill && isItLongLived(aTrack)) { classification = fKill; }
        else if (classification != fKill && killGamma && pdg == 22 && ke < kmaxGamma) {
            classification = fKill;
