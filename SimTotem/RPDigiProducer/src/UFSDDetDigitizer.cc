...
UFSDDetDigitizer::UFSDDetDigitizer(const edm::ParameterSet &params, CLHEP::HepRandomEngine& eng, RPDetId det_id, const edm::EventSetup& iSetup)
...
UFSDDetDigitizer::~UFSDDetDigitizer()
{
...
void UFSDDetDigitizer::run(const std::vector<PSimHit> &input, const std::vector<int> &input_links,
...
{
  if(verbosity_)
    std::cout<<"UFSDDetDigitizer "<<det_id_<<" received input.size()="<<input.size()<<std::endl;
