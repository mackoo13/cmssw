#include "SimTotem/RPDigiProducer/interface/DiamondDigiProducer.h"
...
DiamondDigiProducer::DiamondDigiProducer(const edm::ParameterSet& conf) :
        conf_(conf) {
  ...
  tokenCrossingFrameTotemRP = consumes<CrossingFrame<PSimHit>>(edm::InputTag("mix","g4SimHitsCTPPSHitsDiamond", ""));
...
//	iEvent.getByToken(tokenCrossingFrameTotemRP, cf);
  iEvent.getByLabel("mix", "g4SimHitsCTPPSHitsDiamond", cf);
  ...
  for (simhit_map_iterator it = SimHitMap.begin(); it != SimHitMap.end(); ++it) {
    ...
    if (theAlgoMap.find(it->first) == theAlgoMap.end()) {
      theAlgoMap[it->first] = boost::shared_ptr<DiamondDetDigitizer>(
              new DiamondDetDigitizer(conf_, *rndEngine, it->first, iSetup));
    }
...
void DiamondDigiProducer::beginRun(edm::Run&, edm::EventSetup const& es){
...
void DiamondDigiProducer::endJob() {
}

edm::DetSet<CTPPSDiamondDigi> DiamondDigiProducer::convertRPStripDetSet(const edm::DetSet<RPStripDigi>& rpstrip_detset){
  ...
  /*for(std::vector<RPStripDigi>::const_iterator stripIterator = rpstrip_detset.data.begin(); stripIterator < rpstrip_detset.data.end(); ++stripIterator){
      rpdigi_detset.push_back(CTPPSDiamondDigi());	//todo mackoo13
  }*/
